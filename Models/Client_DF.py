...
class Client_DF(object):
    ...
    def __init__(self,id,original_model,vit,task_per_global_epoch,subset,local_epoch,batch_size,lr,device,method,class_mask):
        self.id = id
        self.vit = vit
    ...
    def get_data(self,task_id,data,mask):
    ...
    def train(self, round, args):
        ...
        print(f'{self.id}号客户端进行第{self.task_id}训练global_epoch中')
        # 先训练global_prompt
        ...
        print('local prompt训练完成！！！')

        ###评估###
        print(f'{self.id}号客户端在{task}号task上的准确率为')
